...
cl "%C2Python%_wrap.c" /I"%PyX64%\include" "%PyX64%\libs\%PyLib%" "ifcengine\lib64\ifcengine.lib" /Fe_%C2Python%.pyd /LD /D "_WINDOWS" /D "WIN64"
copy /y "ifcengine\bin64\ifcengine.dll" "ifcengine.dll"
"%PyX64%\python.exe" "runme.py"

call "%VS%\VC\vcvarsall.bat"
cl "%C2Python%_wrap.c" /I"%PyX32%\include" "%PyX32%\libs\%PyLib%" "ifcengine\lib32\ifcengine.lib" /Fe_%C2Python%.pyd /LD /D "_WINDOWS" /D "WIN32"
